

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=26&duration=2600&pause=700&center=true&vCenter=true&width=900&color=8B5CF6&lines=Mobile+%E2%80%A2+Integra%C3%A7%C3%B5es+%E2%80%A2+Automa%C3%A7%C3%A3o+%E2%80%A2+Telemetria+%E2%80%A2+Dados" />
</p>

<p align="center">
  <img width="100%" src="https://capsule-render.vercel.app/api?type=waving&height=190&color=0:114474,100:8B5CF6&text=João%20Matheus%20de%20Lara&fontColor=ffffff&fontSize=38&fontAlignY=35&desc=Mobile%20%7C%20Integrações%20%7C%20Automação%20%7C%20Telemetria%20%7C%20Dados&descSize=17&descAlignY=58" />
</p>



<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=24&duration=2600&pause=700&center=true&vCenter=true&width=900&color=8B5CF6&lines=Desenvolvimento+Mobile;Integrações+com+APIs;Automação+e+Telemetria;Soluções+reais+para+operações;Software+%2B+Hardware+%2B+Dados" />
</p>

<p align="center">
  <a href="https://www.tsigo.com.br/" target="_blank">
    <img src="https://img.shields.io/badge/TSIGO-site-0B5FFF?style=for-the-badge&logo=google-chrome&logoColor=white" />
  </a>
  <a href="https://www.linkedin.com/in/jo%C3%A3o-matheus-de-lara-629750298/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-Jo%C3%A3o%20Matheus-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://www.instagram.com/" target="_blank">
    <img src="https://img.shields.io/badge/Instagram-@-E4405F?style=for-the-badge&logo=instagram&logoColor=white" />
  </a>
  <a href="https://www.twitch.tv/waynezika" target="_blank">
    <img src="https://img.shields.io/badge/Twitch-waynezika-9146FF?style=for-the-badge&logo=twitch&logoColor=white" />
  </a>
</p>

<br/>

<p align="center">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/php/php-original.svg" height="38" />
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/laravel/laravel-original.svg" height="38" />
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-original.svg" height="38" />
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg" height="38" />
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/java/java-original.svg" height="38" />
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/c/c-original.svg" height="38" />
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/cplusplus/cplusplus-original.svg" height="38" />
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/dart/dart-original.svg" height="38" />
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/flutter/flutter-original.svg" height="38" />
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original.svg" height="38" />
</p>

---

## 👨‍💻 Sobre mim

Atuo no desenvolvimento de soluções tecnológicas completas, integrando **software, hardware e dados** para resolver problemas reais de operação.

Atualmente trabalho na **TSIGO**, atuando no desenvolvimento de sistemas e na busca constante por novas tecnologias voltadas ao mercado de **automação, rastreamento, gestão de frotas, telemetria e monitoramento em tempo real**.

Tenho experiência prática em projetos **em produção**, com foco em **aplicações mobile, integrações, APIs, automações e sistemas operacionais para campo**.

---

## ⚙️ O que eu gosto de desenvolver

<table>
  <tr>
    <td width="50%">
      <h3>🛰️ Rastreamento e telemetria</h3>
      <p>Soluções para monitoramento em tempo real, gestão de frotas, eventos, alertas e dados operacionais.</p>
    </td>
    <td width="50%">
      <h3>📱 Aplicações mobile</h3>
      <p>Aplicativos para operação em campo, motoristas, técnicos, checklists, jornada e sincronização com APIs.</p>
    </td>
  </tr>
  <tr>
    <td width="50%">
      <h3>🔌 Integrações e automação</h3>
      <p>Integrações com equipamentos, sensores, APIs, protocolos e regras automáticas para melhorar processos.</p>
    </td>
    <td width="50%">
      <h3>📊 Dados para operação</h3>
      <p>Transformação de dados em relatórios, indicadores e decisões mais rápidas para empresas.</p>
    </td>
  </tr>
</table>

---

## 💼 Stack em uso prático

### Backend & APIs

![PHP](https://img.shields.io/badge/PHP-777BB4?style=flat&logo=php&logoColor=white)
![Laravel](https://img.shields.io/badge/Laravel-EF3B2D?style=flat&logo=laravel&logoColor=white)
![API](https://img.shields.io/badge/API-REST-lightgrey)
![MySQL](https://img.shields.io/badge/Database-MySQL-4479A1)

### Mobile

![Flutter](https://img.shields.io/badge/Flutter-02569B?style=flat&logo=flutter&logoColor=white)
![Dart](https://img.shields.io/badge/Dart-0175C2?style=flat&logo=dart&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-orange?style=flat&logo=firebase&logoColor=white)
![Offline](https://img.shields.io/badge/Offline--first-success)

### Integrações, IoT e automação

![MQTT](https://img.shields.io/badge/MQTT-IoT%20Messaging-22C55E)
![Telemetria](https://img.shields.io/badge/Telemetria-RealTime-blue)
![Sensores](https://img.shields.io/badge/IoT-Sensores-0AA8A7)
![Automação](https://img.shields.io/badge/Automação-Eventos-orange)
![Hardware](https://img.shields.io/badge/Hardware-Integração-8B5CF6)

### Sistemas embarcados e base técnica

![Arduino](https://img.shields.io/badge/Arduino-00979D?style=flat&logo=arduino&logoColor=white)
![C](https://img.shields.io/badge/C-00599C?style=flat&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=cplusplus&logoColor=white)

---

## 🎓 Base acadêmica e contato técnico

Durante minha formação em **Análise e Desenvolvimento de Sistemas**, tive contato e desenvolvimento de atividades utilizando:

![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white)
![Cypress](https://img.shields.io/badge/Cypress-17202C?style=flat&logo=cypress&logoColor=white)

Essa base contribuiu para minha evolução em **lógica, estrutura de dados, testes automatizados, banco de dados e boas práticas de desenvolvimento**.

---

## 📌 Projetos em destaque

> Alguns projetos são privados por envolverem regras de negócio, clientes, integrações e sistemas internos.

<table>
  <tr>
    <td width="50%">
      <h3>🛰️ Plataforma de rastreamento e gestão de frotas</h3>
      <p>Sistema para monitoramento em tempo real, eventos, alertas, relatórios e gestão operacional de veículos.</p>
      <p>
        <img src="https://img.shields.io/badge/Backend-Laravel-red" />
        <img src="https://img.shields.io/badge/Database-MySQL-4479A1" />
        <img src="https://img.shields.io/badge/Telemetria-RealTime-blue" />
      </p>
    </td>
    <td width="50%">
      <h3>📱 App mobile operacional</h3>
      <p>Aplicativo Flutter para uso em campo, com operação offline, sincronização de dados e integração com APIs.</p>
      <p>
        <img src="https://img.shields.io/badge/Mobile-Flutter-02569B" />
        <img src="https://img.shields.io/badge/API-REST-lightgrey" />
        <img src="https://img.shields.io/badge/Offline--first-success" />
      </p>
    </td>
  </tr>
  <tr>
    <td width="50%">
      <h3>🌡️ Sensores e monitoramento</h3>
      <p>Soluções com sensores de temperatura, umidade, CO₂ e outros dados para controle operacional.</p>
      <p>
        <img src="https://img.shields.io/badge/IoT-Sensores-0AA8A7" />
        <img src="https://img.shields.io/badge/MQTT-Messaging-22C55E" />
        <img src="https://img.shields.io/badge/Dados-Operacionais-8B5CF6" />
      </p>
    </td>
    <td width="50%">
      <h3>🚚 Telemetria e automações</h3>
      <p>Integrações voltadas para eventos, comportamento de condução, alertas, regras automáticas e relatórios.</p>
      <p>
        <img src="https://img.shields.io/badge/Automação-Eventos-orange" />
        <img src="https://img.shields.io/badge/Telemetria-CAN-blue" />
        <img src="https://img.shields.io/badge/Relatórios-Gestão-114474" />
      </p>
    </td>
  </tr>
</table>

---

## 📊 Atividade no GitHub

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=joaomatheusdelara&theme=tokyo-night&hide_border=true&area=true" />
</p>

---

## 🐍 Contribuições

<p align="center">
  <img src="https://raw.githubusercontent.com/joaomatheusdelara/joaomatheusdelara/output/github-contribution-grid-snake.svg" alt="Snake animation" />
</p>

---

## 🧊 Contribuições 3D

<p align="center">
  <img src="./profile-3d-contrib/profile-night-rainbow.svg" alt="3D Contributions" />
</p>

<p align="center">
  <img width="100%" src="https://capsule-render.vercel.app/api?type=waving&height=120&section=footer&color=0:8B5CF6,100:114474" />
</p>
